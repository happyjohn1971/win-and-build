...
// Explicit UK summer-time offset; this sample does not schedule or run a draw.
const sampleDrawAt = Date.parse('2026-09-30T20:00:00+01:00');
function countdownParts(now){const total=Math.max(0,Math.ceil((sampleDrawAt-now)/1000));return [Math.floor(total/86400),Math.floor(total/3600)%24,Math.floor(total/60)%60,total%60]}
function drawCountdown(){return `<section class="draw-countdown" aria-labelledby="draw-clock-title"><div><span class="eyebrow">A DATE FOR YOUR DIARY</span><h2 id="draw-clock-title">Monthly draw countdown</h2><p>Sample draw: <time datetime="2026-09-30T20:00:00+01:00">30 September 2026 · 8pm UK time</time></p><p class="countdown-note">Preview only · Date to be confirmed. No live draw is scheduled.</p></div><div><div id="draw-clock" class="draw-clock" role="timer" aria-live="off">${['Days','Hours','Minutes','Seconds'].map((label,i)=>`<div><strong id="draw-clock-${i}">00</strong><span>${label}</span></div>`).join('')}</div><p id="draw-clock-status" class="countdown-note"></p></div></section>`}
function updateDrawCountdown(){if((location.hash.slice(1)||'/')!=='/')return;const parts=countdownParts(Date.now());parts.forEach((n,i)=>{const el=document.querySelector('#draw-clock-'+i);if(el)el.textContent=String(n).padStart(2,'0')});const status=document.querySelector('#draw-clock-status');if(status)status.textContent=Date.now()>=sampleDrawAt?'Sample date reached — next draw date to be confirmed.':''}
if(typeof window.setInterval==='function')window.setInterval(updateDrawCountdown,1000);

function home(){
  if(!prizes.length){app.innerHTML=`<div class="wrap"><section class="hero"><div class="hero-copy"><div class="eyebrow">FOR THE LOVE OF THE BUILD</div><h1>Small entry.<br><em>Epic possibility.</em></h1><p>Your dream set could be your next build. Discover competitions made for building fans.</p><a class="btn" href="#/competitions">Explore competitions <span>→</span></a></div></section>${drawCountdown()}<div class="trust-strip"><span><b>✓</b> Clear entry limits</span><span><b>◇</b> Sets worth making space for</span><span><b>18+</b> UK residents</span></div>${catalogue()}${how()}</div>`;updateDrawCountdown();return;}
  const featured=prizes[0];
  app.innerHTML=`<div class="wrap"><section class="hero"><div class="hero-copy"><div class="eyebrow">FOR THE LOVE OF THE BUILD</div><h1>Small entry.<br><em>Epic possibility.</em></h1><p>Your dream set could be your next build. Discover competitions made for building fans.</p><a class="btn" href="#/competitions">Explore competitions <span>→</span></a></div><div class="hero-art"><span class="hero-tag">FEATURED PRIZE</span><img src="${img(featured)}" alt="LEGO ${featured.name}"><div class="feature-label">LEGO ${featured.theme}<strong>${featured.name} · ${money(featured.price)} per entry</strong></div></div></section>${drawCountdown()}<div class="trust-strip"><span><b>✓</b> Clear entry limits</span><span><b>◇</b> Sets worth making space for</span><span><b>18+</b> UK residents</span></div>${catalogue()}${how()}</div>`;updateDrawCountdown();
}
...
function info(kind){const content=kind==='faqs'?`<span class="eyebrow">THE USEFUL DETAILS</span><h1>A few good questions.</h1>${faqs.map(([q,a])=>`<details><summary>${q}</summary><p>${a}</p></details>`).join('')}`:`<span class="eyebrow">YOUR INFORMATION</span><h1>Privacy notice.</h1><div class="notice"><strong>Website preview · Draft updated 24 September 2026.</strong> The current website preview practices are explained below. Planned live-service terms are provisional; all bracketed details must be completed before collecting real customer information.</div>
<h2>1. Who is responsible?</h2><p>Win &amp; Build is operated by [OPERATOR LEGAL NAME], the proposed data controller. Business address: [BUSINESS ADDRESS]. Privacy enquiries: [PRIVACY EMAIL]. These details are not yet available; do not send personal information to a placeholder.</p>
<h2>2. This website preview</h2><p>The basket and sample order review are held in memory and clear on refresh. No payment is taken and no real competition entries are allocated. Where enabled, member login uses Auth0 and can process your account identity and email; sign-in is not an anonymous offline demo. The hosting service processes technical requests and login may use session cookies. Product images load from LEGO and fonts from Google Fonts, so those services receive the requests needed to display them. Admin prize changes are stored by the server. The contact, provider and retention details below must be completed before opening competitions.</p>
<h2>3. Information planned for live competitions</h2><p>When the live service is ready, the final notice will cover information supplied through accounts, purchases, postal entries, enquiries and prize claims: full name, postal address, date of birth, email, telephone, competition name, question answer, entry and order records, payment references and, where needed, evidence of eligibility. Payment-card processing will be handled by [PAYMENT PROVIDER]; the provider and data handled must be confirmed before launch. Technical security data and any information received from service providers must also be specified.</p>
<h2>4. Planned purposes and lawful bases</h2><p>The proposed uses are administering accounts and valid paid or free entries, contacting winners and delivering prizes (performance of the competition contract); preventing abuse and resolving enquiries or disputes (legitimate interests in fair and secure competitions); and keeping records required by law (legal obligation). Optional promotional emails would require a separate, freely given choice and a simple unsubscribe method. Entry will not depend on agreeing to marketing. [CONFIRM EACH PURPOSE, LAWFUL BASIS AND ANY APPLICABLE LEGAL OBLIGATION BEFORE LAUNCH].</p>
<h2>5. Sharing and international transfers</h2><p>The final notice will identify the necessary hosting, payment, email, delivery and professional service providers, what they receive and why. Information may also need to be disclosed where legally required. [CONFIRM PROVIDERS, RECIPIENT CATEGORIES, PROCESSING COUNTRIES AND ANY INTERNATIONAL-TRANSFER SAFEGUARDS]. Current hosting, external image/font requests and enabled authentication are described above; competition payments remain disabled.</p>
<h2>6. Retention and security</h2><p>The preview basket clears when the page refreshes; this does not delete authentication-provider or server records. Before launch we will publish retention periods or clear criteria for accounts, paid and postal entries, order records, eligibility documents, correspondence and draw records: [RETENTION SCHEDULE]. The live service must have appropriate access controls and safeguards. Administrative access is protected by the existing server login and access checks.</p>
<h2>7. Your rights</h2><p>Depending on the circumstances, you may request access, correction, deletion, restriction or portability of your personal information. Where processing relies on consent, you can withdraw it without affecting earlier lawful processing.</p><p><strong>Right to object:</strong> you may object to processing based on legitimate interests for reasons relating to your situation. You can object to direct marketing at any time.</p><p>Use [PRIVACY EMAIL] once the operator has supplied a working contact address. You can also complain to the UK Information Commissioner’s Office through <a href="https://ico.org.uk/make-a-complaint/" target="_blank" rel="noopener noreferrer">the ICO complaints service</a>. Rights can be subject to legal exceptions.</p>
<h2>8. Winners, eligibility and automated decisions</h2><p>The intended service is for people aged 18 or over. Only evidence necessary to check eligibility should be requested. [CONFIRM WINNER-PUBLICATION DETAILS, PURPOSE, LAWFUL BASIS AND OBJECTION PROCESS]. Optional winner photos or promotional interviews must be addressed separately. No live prize draw is enabled in this website preview. The final notice must explain any live automated decision-making or profiling that applies.</p>
<h2>9. Required information and cookies</h2><p>The final entry form will distinguish information required to administer an entry from optional information. Missing required details may prevent validation or prize delivery. The live cookie notice and controls must reflect the technologies actually installed; [CONFIRM LIVE COOKIES AND ANY CONSENT REQUIREMENTS]. This preview does not enable marketing or analytics cookies.</p>
<h2>10. Updates</h2><p>A completed notice will be available before live customer data is collected, with its effective date and material changes explained. This draft describes the preview and planned competition features; no payment service is active.</p><p><a class="btn" href="#/competitions">Back to competitions</a></p>`;app.innerHTML=`<div class="wrap page info">${content}</div>`;}
